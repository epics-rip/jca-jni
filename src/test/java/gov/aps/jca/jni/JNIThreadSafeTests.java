...
@RunWith(Suite.class)
@Suite.SuiteClasses({ JCAChannelIT.class, JCAMonitorIT.class, JCAContextPendPoolFlushIT.class, JCAChannelGetIT.class,
		JCAChannelPutIT.class })
public class JNIThreadSafeTests {
	/**
	 *
	 */
	@BeforeClass
	public static void setup() {
		try {
			...
			prefs.put(JCAIT.JNIType, JCALibrary.JNI_THREAD_SAFE);
			prefs.flush();
			initialize();
		} catch (CAException e) {
			e.printStackTrace();
	...
	private static void registerProcessVariables(DefaultServerImpl server) {
		...
		MemoryProcessVariable enumPV = new MemoryProcessVariable("enum", null, DBR_Enum.TYPE, new short[] { 0 }) {
			private final String[] labels = { "zeroString", "oneString", "twoString", "threeString", "fourString",
					"fiveString", "sixString", "sevenString", "8s", "9s", "10s", "11s", "12s", "13s", "14s", "15s" };
